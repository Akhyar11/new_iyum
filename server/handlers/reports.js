...
export const INITIAL_PACKAGE_REPORT_STATS = [
  {
    code: 'PKG-ALL-IN',
    ...
    percentage: 46.8,
    price: 7500000,
    totalRevenuePotential: 160 * 7500000, // Rp 1.200.000.000
    trend: '+12.5% bln ini',
  ...
  },
  {
    ...
    percentage: 25.1,
    price: 3500000,
    totalRevenuePotential: 86 * 3500000, // Rp 301.000.000
    trend: '+5.2% bln ini',
  ...
  },
  {
    ...
    percentage: 19.9,
    price: 5000000,
    totalRevenuePotential: 68 * 5000000, // Rp 340.000.000
    trend: '+2.1% bln ini',
  ...
  },
  {
    ...
    percentage: 8.2,
    price: 2500000,
    totalRevenuePotential: 28 * 2500000, // Rp 70.000.000
    trend: '+1.4% bln ini',
...
/**
 * GET: Mengambil statistik paket paling diminati beserta persentase rekomendasi
 * Mendukung filter periode: 'all', 'month', 'quarter', 'year'
 */
export async function handleGetPackagePopularityStats(req, res) {
  ...
  // Multiplier untuk simulasi rentang periode
  let factor = 1.0;
  ...
  if (supabaseUrl && supabaseAnonKey) {
    try {
      ...
      if (!recError && recs && recs.length > 0 && !pkgError && pkgs && pkgs.length > 0) {
        ...
        recs.forEach(r => {
          const code = r.top_package_code || (pkgs.find(p => p.id === r.top_package_id)?.code);
          if (code) {
            countsByCode[code] = (countsByCode[code] || 0) + 1;
          }
        });

        const aggregated = pkgs.map(p => {
        ...
        if (aggregated.some(a => a.count > 0)) {
          baseStats = aggregated;
        }
      }
    } catch (err) {
      console.warn('Database error in handleGetPackagePopularityStats:', err.message);
    }
  }
